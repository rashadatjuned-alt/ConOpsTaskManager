...
export default function AllProjects() {
  ...
  if (loading) return <AppShell title="All Projects"><div style={{ padding: 40, color: '#6b7280', textAlign: 'center' }}>Loading…</div></AppShell>

  return (
    <AppShell title="All Projects">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <div style={{ fontSize: 11, color: '#4b5563' }}>{projects.length} project{projects.length !== 1 ? 's' : ''}</div>
          ...
          <div key={proj.id} style={S.projCard}>
            <div style={S.projHeader}>
              ...
              <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                ...
                {canDelete && (
                  <button onClick={() => deleteProject(proj)} style={{ ...S.iconBtn, color: '#f87171', borderColor: 'rgba(239,68,68,0.2)' }}>🗑</button>
                )}
              </div>
            </div>
            ...
            {isOpen && (
              <div style={S.taskList}>
                <div style={S.divider} />
                {pt.length === 0 ? (
                  <div style={{ fontSize: 12, color: '#4b5563', padding: '6px 0' }}>No tasks yet.</div>
                ) : pt.map((t: any) => {
...
const S: Record<string, React.CSSProperties> = {
  ...
  projHeader: { display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', background: '#111' },
  projName:   { fontSize: 13, fontWeight: 600, color: '#f3f4f6', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', minWidth: 0 },
  ...
  projPct:    { fontSize: 11, fontWeight: 600, color: '#6b7280', fontFamily: 'monospace' },
  taskList:   { padding: '0 14px 12px' },
  divider:    { height: 1, background: 'rgba(255,255,255,0.05)', marginBottom: 8 },
  taskRow:    { display: 'flex', alignItems: 'center', gap: 8, padding: '7px 8px', borderRadius: 6, marginBottom: 4, border: '1px solid rgba(255,255,255,0.04)', transition: 'background 0.12s' },
  ...
  subBadge:   { fontSize: 10, color: '#4b5563', background: '#0a0a0a', padding: '1px 6px', borderRadius: 8, whiteSpace: 'nowrap', flexShrink: 0 },
  iconBtn:    { background: 'rgba(255,255,255,0.05)', color: '#9ca3af', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 5, padding: '3px 8px', fontSize: 11, cursor: 'pointer', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', fontFamily: 'inherit' },
  editBtn:    { background: 'rgba(255,255,255,0.05)', color: '#9ca3af', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 5, padding: '2px 7px', fontSize: 11, cursor: 'pointer', flexShrink: 0 },
  delBtn:     { background: 'none', border: 'none', color: '#f87171', cursor: 'pointer', fontSize: 13, padding: '2px 4px', flexShrink: 0 },
}
