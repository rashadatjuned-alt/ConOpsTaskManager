...
export default function TaskDetail({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (!tData) { router.push('/dashboard'); return }
      setTask(tData)
  ...
  return (
    <AppShell title="Task Context">
      ...
      <style jsx>{`
        .task-layout { display: flex; gap: 24px; align-items: flex-start; }
        .main-col { flex: 2; }
        .side-col { flex: 1; display: flex; flex-direction: column; gap: 24px; }

        .detail-card { background: var(--card-bg); border: 1px solid var(--border); border-radius: 12px; padding: 24px; }
        
        .d-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 24px; padding-bottom: 24px; border-bottom: 1px solid var(--border); }
        .d-title { font-size: 20px; font-weight: 800; color: var(--txt-main); margin: 0; max-width: 80%; line-height: 1.3; }
        ...
        .meta-card { background: var(--card-bg); border: 1px solid var(--border); border-radius: 12px; padding: 24px; }
        .meta-item { display: flex; gap: 12px; margin-bottom: 16px; }
        .meta-item:last-child { margin-bottom: 0; }
        .meta-row { display: flex; gap: 16px; margin-top: 16px; padding-top: 16px; border-top: 1px solid var(--border); }
