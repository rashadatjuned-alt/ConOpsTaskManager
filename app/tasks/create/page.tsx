...
export default function CreateTask() {
  ...
  const [allUsers,       setAllUsers]       = useState<any[]>([])   // non-Admin users only
  ...
  useEffect(() => {
    const load = async () => {
      const [p, u] = await Promise.all([
        ...
        // Fetch with role so we can filter out Admins
        supabase.from('Users').select('id,full_name,email,role'),
      ])
      setAllProjects(p.data || [])
      // Exclude Admin users from assignee list
      setAllUsers((u.data || []).filter((usr: any) => usr.role !== 'Admin'))
    }
    load()
  }, [])

  // Load project members when project changes — also exclude Admins
  useEffect(() => {
