...
import { 
  ChevronRight, LayoutList, Columns, Plus, Filter, Users, ChevronsUpDown
} from 'lucide-react'
...
export default function AllTasks() {
  ...
  const filteredTasks = useMemo(() => {
    return tasks.filter(t => {
      const projMatch = pf === 'All' || t.project_name === pf
      const assignMatch = af === 'All' || (t.task_assignees || []).some((ta: any) => ta.user_id === af)
      return projMatch && assignMatch
    })
  }, [tasks, pf, af])
  ...
  const filteredSubtasks = useMemo(() => {
    return subtasks.filter(st => {
      ...
      const projMatch = pf === 'All' || parent?.project_name === pf
      const assignMatch = af === 'All' || (st.subtask_assignees || []).some((sa: any) => sa.user_id === af)
      return projMatch && assignMatch
    })
  }, [subtasks, tasks, pf, af])
  ...
  if (loading) return <AppShell title="All Tasks"><div style={{ padding: 40, textAlign: 'center' }}>Loading collective dashboards...</div></AppShell>
    ...
    <AppShell title="All Tasks Oversight">
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg2)', border: '1px solid var(--brd)', borderRadius: 10, padding: '6px 12px' }}>
          ...
          <select value={af} onChange={e => setAf(e.target.value)} style={{ background: 'transparent', border: 'none', color: 'var(--txt)', fontSize: 13, outline: 'none', fontWeight: 600, cursor: 'pointer' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 12 }}>
          ...
          <Link href="/tasks/create" className="btn-primary-action" style={{ textDecoration: 'none', background: 'var(--txt)', color: 'var(--bg)', padding: '8px 16px', borderRadius: 10, display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 700 }}><Plus size={14} /> Create Task</Link>
        </div>
      </div>

      {view === 'list' ? (
        groupedTasks.length === 0 ? (
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--txt3)', border: '1px dashed var(--brd)', borderRadius: 12 }}>No execution logs match your active dashboard filter.</div>
        ) : (
