...
export default function AllTasks() {
  ...
  const filteredTasks = useMemo(() => {
    return tasks.filter(t => 
      (pf === 'All' || t.project_name === pf) && 
      (af === 'All' || (t.owner || '').includes(af) || (t.assignees || '').includes(af))
    )
  }, [tasks, pf, af])
  ...
  const filteredSubtasks = useMemo(() => {
    return subtasks.filter(st => {
      ...
      const assigneeMatch = af === 'All' || (st.owner || '').includes(af) || (st.assignees || '').includes(af)
    ...
    <AppShell title="All Tasks Oversight">
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg2)', border: '1px solid var(--brd)', borderRadius: 10, padding: '6px 12px' }}>
          ...
          <select value={af} onChange={e => setAf(e.target.value)} style={{ background: 'transparent', border: 'none', color: 'var(--txt)', fontSize: 13, outline: 'none', fontWeight: 600 }}>
      ...
      {view === 'list' ? (
        ...
        <KanbanBoard 
          mode={kanbanMode} 
          tasks={filteredTasks} 
          allTasks={tasks} // Pass raw un-filtered tasks to resolve subtask parent names 
