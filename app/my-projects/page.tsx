...
import { ChevronRight, Info, X, Calendar, Clock, Search, ChevronsUpDown, Users } from 'lucide-react'
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 6 }}>
              {['Not Started', 'In Progress', 'On-Hold', 'Completed'].map(st => (
                <div key={st} style={{ background: 'var(--bg2)', padding: '8px 4px', borderRadius: 'var(--r)', textAlign: 'center' }}>
                  <div style={{ fontSize: 16, fontWeight: 800 }}>{projTasks.filter(t => t.status === st).length}</div>
                  <div style={{ fontSize: 8, color: 'var(--txt3)', fontWeight: 700 }}>{st.toUpperCase()}</div>
                </div>
              ))}
            </div>
          </div>
  ...
  const myFilteredProjects = useMemo(() => {
    return projects
      .filter(proj => {
        ...
        return (isMember || hasTask || hasSub) && proj.name.toLowerCase().includes(searchTerm.toLowerCase())
      })
      .sort((a, b) => a.name.localeCompare(b.name))
  }, [projects, tasks, subtasks, me, searchTerm])
    ...
    <AppShell title="My Projects Portfolio">
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 20 }}>
        <div style={{ position: 'relative', width: 280 }}>
          ...
          <input type="text" placeholder="Search my projects..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} 
