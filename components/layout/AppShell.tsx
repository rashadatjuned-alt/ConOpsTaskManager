...
export default function AppShell({ children, title }: { children: React.ReactNode, title: string }) {
  ...
  if (loading) return <div style={{ background: 'var(--bg3)', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--txt3)' }}>Loading...</div>

  return (
    <div className="app-layout">
      <aside className="sidebar">
        {/* LOGO AREA: "The Node" Implementation */}
        ...
        <div className="sidebar-footer">
          ...
          <div className="footer-actions">
            ...
            <button onClick={async () => { await supabase.auth.signOut(); router.push('/auth') }} className="icon-btn logout"><LogOut size={16} /></button>
          </div>
        </div>
      </aside>
      ...
      <style jsx>{`
        ...
        .nav-container { flex: 1; padding: 0 16px; display: flex; flex-direction: column; gap: 2px; overflow-y: auto; }
        ...
        .nav-item { display: flex; align-items: center; gap: 12px; padding: 10px 16px; color: var(--txt2); text-decoration: none; font-size: 13.5px; font-weight: 600; border-radius: 8px; transition: 0.2s; }
        ...
        .icon-btn.logout:hover { color: #ef4444; background: rgba(239, 68, 68, 0.05); }
