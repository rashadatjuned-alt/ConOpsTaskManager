...
export default function AppShell({ children, title }: { children: React.ReactNode, title: string }) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <style jsx>{`
        .nav-label { padding: 10px 12px; font-size: 10px; font-weight: 800; color: #333; text-transform: uppercase; letter-spacing: 1.5px; }
        .nav-link { display: flex; align-items: center; gap: 12px; padding: 12px 16px; color: #777; text-decoration: none; font-size: 14px; font-weight: 600; border-radius: 12px; transition: 0.2s; }
        .nav-link:hover { color: #fff; background: #111; }
        .nav-link.active { color: #fff; background: #161616; box-shadow: inset 0 0 0 1px #222; }
        .util-btn { background: #111; border: 1px solid #222; color: #555; padding: 10px; border-radius: 10px; cursor: pointer; transition: 0.2s; display: flex; align-items: center; justify-content: center; }
        .util-btn:hover { background: #1a1a1a; color: #fff; border-color: #444; }
        .logout-hover:hover { color: #ef4444; background: rgba(239, 68, 68, 0.05); border-color: rgba(239, 68, 68, 0.2); }
      `}</style>
    </div>
  )
}
